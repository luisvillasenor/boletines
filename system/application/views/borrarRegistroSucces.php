<!-- this section shows you most of the styled elements from the general stylesheet -->
<section class="row">
    <!-- MENU ADMINISTRACION -->
    <?php include "system/application/views/menu_operaciones.php";?>
    <h2 class="width_16 col"></h2>
    <div class="col col_14">
        <article>
        <h1>Registro eliminado</h1><small></small>
        </br>
        <blockquote class="right">El registro fué ELIMINADO con éxito !!!<p>Verifique en la Bitácora de Seguimientos Escritos</p></blockquote>
        </article>        
    </div><!-- col_11 -->
</section><!-- row -->
